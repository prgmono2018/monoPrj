...
function _ensureProjectConfig() {
  _ensureProjectConfig = _asyncToGenerator(function* (context) {
    ...
    const projectManifest = yield _fsExtra.default.readJson(manifestPath);
    ...
    if (!projectId) {
      const selected = yield getOrCreateProject(context);
    ...
    if (!dataset) {
      ...
      const projectContext = _objectSpread({}, context, {
        apiClient
      });
...
function _getOrCreateProject() {
  _getOrCreateProject = _asyncToGenerator(function* (context) {
    ...
    if (projects.length === 0) {
      ...
      const projectName = yield prompt.single({
