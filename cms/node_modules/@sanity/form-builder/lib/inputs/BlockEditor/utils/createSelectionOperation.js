...
function createSelectionOperation(change) {
  const focusPath = change.value.selection.focus && change.value.selection.focus.key ? change.value.document.getPath(change.value.selection.focus.key) : undefined;
  const anchorPath = change.value.selection.anchor && change.value.selection.anchor.key ? change.value.document.getPath(change.value.selection.anchor.key) : undefined;
  const props = {
    type: 'set_selection',
    value: change.value,
