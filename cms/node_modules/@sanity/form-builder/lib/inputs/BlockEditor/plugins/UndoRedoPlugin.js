...
function shouldSquashOperations(operations) {
  const operationsPaths = (0, _uniq2.default)(operations.map(op => op.path ? JSON.stringify(op.path.toJSON()) : null).toArray().filter(Boolean));
    ...
    item.remoteOperations.forEach(remoteOp => {
      ...
      switch (remoteOp.type) {
        case 'insert_node':
          ...
          if (pAbove || pEqual) {
            const newPath = _slate.PathUtils.increment(checkedOp.path, 1, pIndex);
        ...
        case 'remove_node':
          ...
          if (pAbove) {
            const newPath = _slate.PathUtils.decrement(checkedOp.path, 1, pIndex);
...
function UndoRedoPlugin(options) {
  ...
  function handleUndoItem(editor, item, _operations) {
    ...
    let operations = (0, _immutable.List)(_operations || []);
    const undoOperations = item.remoteOperations.size === 0 ? item.operations : recalculateOperations(item);
  ...
  function handleRedoItem(editor, item, _operations) {
    ...
    let operations = (0, _immutable.List)(_operations || []);
    const redoOperations = item.remoteOperations.size === 0 ? item.operations : recalculateOperations(item);
