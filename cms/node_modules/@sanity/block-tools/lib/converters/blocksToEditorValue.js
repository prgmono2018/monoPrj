...
function sanitySpanToRawSlateBlockNode(span, sanityBlock, blockContentFeatures, childIndex) {
  ...
  const annotationKeys = marks.filter(mark => decorators.indexOf(mark) === -1 && sanityBlock.markDefs.map(def => def._key).includes(mark));
  let annotations;
...
function sanityBlockToRawNode(sanityBlock, blockContentFeatures, options = {}) {
  ...
  if (hasKeys(rest)) {
    ...
    const listItem = restData.data.listItem;
    ...
    const style = restData.data.style;
...
function sanityBlockItemToRaw(blockItem, blockContentFeatures) {
  ...
  const type = blockContentFeatures.types.blockObjects.map(objType => objType.name).concat('block').includes(blockItem._type) ? blockItem._type : '__unknown';
  return {
    object: 'block',
    key: blockItem._key,
    type,
...
function blocksToEditorValue(array, type, options = {}) {
  const blockContentFeatures = (0, _blockContentTypeToOptions.default)(type);
  return {
    object: 'value',
    data: {},
    document: {
