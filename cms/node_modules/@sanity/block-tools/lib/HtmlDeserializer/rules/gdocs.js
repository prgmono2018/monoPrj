...
const LIST_CONTAINER_TAGS = Object.keys(_constants.HTML_LIST_CONTAINER_TAGS); // font-style:italic seems like the most important rule for italic / emphasis in their html
...
function getBlockStyle(el, enabledBlockStyles) {
  const block = blocks[(0, _helpers.tagName)(el.firstChild)];
...
function createGDocsRules(blockContentType, options = {}) {
  return [{
    deserialize(el, next) {
      if ((0, _helpers.tagName)(el) === 'span' && isGoogleDocs(el)) {
        const span = _objectSpread({}, _constants.DEFAULT_SPAN, {
          marks: [],
          text: el.innerText
        });
