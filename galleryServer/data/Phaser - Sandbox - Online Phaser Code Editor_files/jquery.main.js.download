...
// initFixIframe
function initFixIframe() {
	var holder = jQuery('.featured-game');
	var win = jQuery(window);
	function iframeHeight() {
		var iframe = holder.find('iframe');
		// iframe.css({height:iframe.outerWidth()/2+170});
	}
	iframeHeight();
	win.on('resize', iframeHeight);
}
...
// popups init
function initPopups() {
	jQuery('.features-holder').contentPopup({
		mode: 'hover',
		popup: '.popup1',
		btnOpen: '.open1'
	});
...
function initSameHeight() {
	jQuery('.guide-holder').sameHeight({
		elements: '.box',
		skipClass: 'guide-box',
		flexible: true,
		multiLine: true
	});
	jQuery('.slideshow .mask').sameHeight({
		elements: '.slide'
	});
	jQuery('.showcase').sameHeight({
		elements: '> li',
		flexible: true,
		multiLine: true
	});
...
function initChildClasses() {
	jQuery('#content table tbody').children(':last-child').addClass('last-child');
}
